...
int main(int argc, char *argv[]) {
  ...
  /**
   * Ugly hack to allow us to attach with a ssh-based debugger to the
   * application.
   */
  int do_sleep = 0;
  while (do_sleep) {
  ...
  rc = jacobi_cpu(om, NB, MB, P, Q, MPI_COMM_WORLD, epsilon, max_iter,
                  save_output);
  if (rc < 0) {
